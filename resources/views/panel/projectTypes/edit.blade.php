...
@section('title', 'Edit projectTypes')
...
<h3> {{__('Edit')}} {{__('Cases')}}</h3>
@endsection

@section('breadcrumb-items')
<li class="breadcrumb-item">{{__('Cases')}}</li>
<li class="breadcrumb-item active"> {{__('Edit')}} {{__('Cases')}}</li>
@endsection
...
<div class="container-fluid">
    <div class="edit-profile">
        <div class="row">
            ...
            <div class="col-xl-12">
                <form method="POST" action="{{ route('projectTypes.update', $projectType->id) }}"
                    enctype="multipart/form-data" class="card">

                    @csrf
                    @method('PUT')
                    <div class="card-header">
                        {{-- <h4 class="card-title mb-0">{{ __('Edit projectType') }}</h4> --}}
                        ...
                        <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select required name="country_id[]" id="country_id" class="form-control select2"
                                        ...
                                        <option value="{{ $country->id }}" {{ in_array($country->id,
                                            $projectType->country_id) ? 'selected' : '' }}>{{ $country->name }}
                                        </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="checkbox" id="checkbox2" >{{ __('All') }}
                                    ...
                                    <select required name="city_id[]" id="city_id" class="form-control select2"
                                        ...
                                        <option value="{{ $city->id }}" {{ in_array($city->id, $projectType->city_id) ?
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <img id="image-preview" src="{{$projectType->photo}}"
                             ...
                             @for($i=0;$i< count($array_data);$i++) <div class="card">
                                <div id="idiom-fields" class="row">
                                    <div id="idiom-{{$array_data[$i][0]->idiom_id}}">
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                <label for="projectType_{{$array_data[$i][0]->idiom_id}}"
                                                    class="form-label">{{ __('Title') }}</label>

                                                <div class="col-md-12">
                                                    <input id="projectType_{{$array_data[$i][0]->idiom_id}}" type="text"
                                                        class="form-control @error('projectType_es') is-invalid @enderror"
                                                        name="projectType_{{$array_data[$i][0]->idiom_id}}"
                                                        value="{{$array_data[$i][0]->projectType}}" autocomplete="projectType_es"
                                                        autofocus>
